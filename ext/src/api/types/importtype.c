...
PHP_FUNCTION (wasm_importtype_new) {
    ...
    wasm_byte_vec_new(module_vec, module_len, module);

    wasm_name_t *name_vec = malloc(sizeof(wasm_name_t));
    wasm_byte_vec_new(name_vec, name_len, name);

    wasmer_res *externtype_res = WASMER_RES_P(externtype_val);

    wasmer_res *importtype = emalloc(sizeof(wasmer_res));
    importtype->inner.importtype = wasm_importtype_new(module_vec, name_vec, wasm_externtype_copy(WASMER_RES_INNER(externtype_res, externtype)));
