...
zend_object *wasm_extern_vec_create(zend_class_entry *ce)
{
    wasm_extern_vec_c *wasm_extern_vec = zend_object_alloc(sizeof(wasm_extern_vec_c), ce);

    zend_object_std_init(&wasm_extern_vec->std, ce);
    wasm_extern_vec->std.handlers = &wasm_extern_vec_object_handlers;

    return &wasm_extern_vec->std;
}
void wasm_extern_vec_free(zend_object *object) {
    wasm_extern_vec_c *vec = (wasm_extern_vec_c*) ((char *)(object) - XtOffsetOf(wasm_extern_vec_c, std));

    if (vec->vec.inner.xtern) {
        if (vec->vec.owned && vec->vec.inner.xtern->data) {
            free(vec->vec.inner.xtern->data);
            vec->vec.inner.xtern->data = NULL;
            vec->vec.inner.xtern->size = 0;
        }

        if (vec->vec.owned || vec->vec.allocated) {
            efree(vec->vec.inner.xtern);
        }
    }

    vec->vec.inner.xtern = NULL;
...
PHP_MINIT_FUNCTION(wasm) {
    ...
    // Engines (deprecated — engine types eliminated in Wasmer v4+, kept for backward compat)
    REGISTER_LONG_CONSTANT("WASM_ENGINE_JIT", 0, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("WASM_ENGINE_NATIVE", 1, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("WASM_ENGINE_OBJECT_FILE", 2, CONST_CS | CONST_PERSISTENT);

    // Backends (formerly "compilers")
    ...
    REGISTER_LONG_CONSTANT("WASM_ANYREF", WASM_EXTERNREF, CONST_CS | CONST_PERSISTENT);
