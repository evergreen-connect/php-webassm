...
.PHONY: ext/download
ext/download: ## Download Wasmer library for current platform
	ext/download-wasmer.sh

WASMER_LIB := $(if $(filter Darwin,$(shell uname -s)),ext/lib/libwasmer.dylib,ext/lib/libwasmer.so)
...
ext/Makefile: ext/config.m4 ext/Makefile.frag | $(WASMER_LIB)
ifneq (,$(PHP_HOME))
